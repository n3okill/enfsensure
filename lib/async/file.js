...
function ensure(path, opt, callback) {
    ...
    options.fs.stat(path, function(err, stat) {
        if (err) {
...
function createFile(file, options, callback) {
    ensureDir(nodePath.dirname(file), {fs: options.fs, mode: options.dirMode}, function(err) {
        if (err) {
            return callback(err);
        }
        if (options.stream) {
...
function ensureWriteFile(file, options, callback) {
    options.fs.open(file, options.append ? "a+" : "wx", options.mode, function(errOpen, fd) {
        if (errOpen) {
            return callback(errOpen);
        }
        if (options.hasOwnProperty("data")) {
            options.fs.write(fd, options.data, options.encoding, function(errWrite) {
                if (errWrite) {
